...
setup_nexus_ci_variables() {
    ...
    local -a ci_vars=(
        ...
        # HOME to the build dir). Cluster trust lives in the kubeconfig itself
        # (insecure-skip-tls-verify for the k3d self-signed certs).
        "KUBECONFIG|/root/.kube/config|false"
        # Infrastructure referential: endpoints, transport posture, org policy
        # and ssh host-key stance all come from the mounted P-lab instance
        # (replaces BRIK_SSH_STRICT_HOST_KEY, BRIK_KUBECTL_OPTS,
        # BRIK_POLICY_URL, ARGOCD_SERVER and the BRIK_COSIGN_* knobs).
        "BRIK_INFRA_DIR|/etc/brik/infra|false"
    )

    # Gitea API token backing the referential's git-api credential
    # (env://BRIK_GIT_TOKEN): branch-protection checks on the evidence store.
    if [[ -n "${GITEA_PAT:-}" ]]; then
        ci_vars+=("BRIK_GIT_TOKEN|${GITEA_PAT}|true")
    else
        log_warn "GITEA_PAT not found in .env -- run gitea setup, then re-run this setup"
    fi

    # ArgoCD (GitOps deploy): the server address and TLS posture come from the
    # referential's ArgoCD endpoint; only the non-expiring API token generated
    # by k3d.sh setup (stored in .env) travels as a secret variable.
    ...
    # Cosign signing material for the node-deploy-signed scenario. The
    # referential's Signing endpoint references env://COSIGN_PRIVATE_KEY for
    # sign and trust/cosign.pub for verify; only the secret material travels
    # as group variables.
