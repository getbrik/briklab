...
source "${_RECOVERY_DIR}/auth/jenkins-trust.sh"
...
# Propagate the rotating tokens to the 'brik' GitLab group's CI variables
# (create or update): ARGOCD_AUTH_TOKEN (generated by k3d setup) and
# BRIK_GIT_TOKEN (the Gitea PAT backing the referential's git-api credential,
# regenerated whenever Gitea is rebuilt). gitlab.sh seeds these at setup
# time, but on a fresh init it runs BEFORE gitea.sh and k3d.sh produce the
# final values -- this propagation is what converges them. No-op when the
# group is absent.
...
# Recreate Jenkins when a token baked into its container env no longer
# matches .env: ARGOCD_AUTH_TOKEN (deploy asserts) and GITEA_PAT (SCM scans,
# webhook management, the casc BRIK_GIT_TOKEN seed). Compose env is fixed at
# container CREATE time, so convergence requires a recreate -- which wipes
# the lab-CA truststore import, hence the re-trust afterwards.
briklab.recover.jenkins_token() {
    ...
    local container_token container_pat
    container_token=$(docker exec brik-jenkins printenv ARGOCD_AUTH_TOKEN 2>/dev/null || echo "")
    container_pat=$(docker exec brik-jenkins printenv GITEA_PAT 2>/dev/null || echo "")

    if [[ -n "${ARGOCD_AUTH_TOKEN:-}" && "$container_token" == "$ARGOCD_AUTH_TOKEN" ]] \
        && [[ -n "${GITEA_PAT:-}" && "$container_pat" == "$GITEA_PAT" ]]; then
        log_ok "Jenkins tokens match .env"
        return 0
    fi

    log_warn "Jenkins tokens outdated -- recreating..."
    ...
    if briklab.wait.until 100 5 check_http "${jenkins_url}/login"; then
        log_ok "Jenkins recreated with updated tokens"
    else
        log_warn "Jenkins slow to start -- it may need a few more seconds"
    fi

    # The recreate dropped the writable layer: re-import the lab CA and
    # restart so the JVM reloads its truststore.
    briklab.auth.jenkins_trust_ca --restart
    briklab.wait.until 100 5 check_http "${jenkins_url}/login" || true
}
